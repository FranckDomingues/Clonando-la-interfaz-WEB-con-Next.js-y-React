...
const BookingPanel = ({ room, compact = false }: BookingPanelProps) => {
    ...
    return (
      <section className="flex items-center justify-between gap-3 bg-white text-sm">
        <div className="min-w-0">
          <p><span className="mr-1 text-neutral-500 line-through">{room.originalPrice}</span><strong>{room.currentPrice}</strong></p>
          <p className="truncate text-xs text-neutral-600">{room.stayText} · Cancelación gratuita</p>
        </div>
  ...
  return (
    <section className="rounded-2xl border border-neutral-200 bg-white p-5 shadow-lg md:sticky md:top-6">
      <div className="flex items-baseline gap-2"><span className="text-sm text-neutral-500 line-through">{room.originalPrice}</span><strong className="text-xl">{room.currentPrice}</strong></div>
      <p className="mt-1 text-sm text-neutral-600">{room.stayText}</p>
